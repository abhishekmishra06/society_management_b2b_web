...
export async function POST(request) {
  try {
    ...
    await pool.execute('INSERT INTO complaints (id,title,description,category,status,priority,reportedBy,flatNumber,assignedTo) VALUES (?,?,?,?,?,?,?,?,?)',
      [id, body.title||'', body.description||'', body.category||'general', body.status||'open', body.priority||'medium', body.reportedBy||'', body.flatNumber||'', body.assignedTo||'']);
...
export async function PUT(request) {
  try {
    ...
    ['title','description','category','status','priority','reportedBy','flatNumber','assignedTo'].forEach(k=>{if(body[k]!==undefined){fields.push(`${k}=?`);values.push(body[k]);}});
    if(fields.length>0){values.push(body.id);await pool.execute(`UPDATE complaints SET ${fields.join(',')} WHERE id=?`,values);}
