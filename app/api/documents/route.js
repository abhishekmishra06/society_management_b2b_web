...
export async function POST(request) {
  try {
    ...
    await pool.execute('INSERT INTO documents (id,title,category,description,fileUrl,fileName,fileSize,uploadedBy,status) VALUES (?,?,?,?,?,?,?,?,?)',
      [id, body.title||'', body.category||'general', body.description||'', body.fileUrl||'', body.fileName||'', body.fileSize||'', body.uploadedBy||'', body.status||'active']);
...
export async function PUT(request) {
  try {
    ...
    ['title','category','description','fileUrl','fileName','status'].forEach(k=>{if(body[k]!==undefined){fields.push(`${k}=?`);values.push(body[k]);}});
    if(fields.length>0){values.push(body.id);await pool.execute(`UPDATE documents SET ${fields.join(',')} WHERE id=?`,values);}
