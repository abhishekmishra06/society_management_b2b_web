...
export async function GET(request) {
  try {
    ...
    const [rows] = await pool.execute('SELECT * FROM residents ORDER BY createdAt DESC');
...
export async function POST(request) {
  try {
    ...
    await pool.execute(
      ...
      [id, body.name||'', body.flatNumber||'', body.tower||'', body.phone||'', body.email||'', body.type||'owner', body.status||'active', body.vehicleNumber||'', body.aadhaarNumber||'', body.moveInDate||null, body.emergencyContact||'']
    );
...
export async function PUT(request) {
  try {
    ...
    ['name','flatNumber','tower','phone','email','type','status','vehicleNumber','aadhaarNumber','moveInDate','emergencyContact'].forEach(k => { if(body[k]!==undefined){fields.push(`${k}=?`);values.push(body[k]);} });
    if(fields.length>0){values.push(body.id); await pool.execute(`UPDATE residents SET ${fields.join(',')} WHERE id=?`,values);}
