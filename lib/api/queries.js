...
export const useUpdateParkingSlot = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...slotData }) => {
      const { data } = await apiClient.put(`/parking/${id}`, slotData);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.PARKING });
...
export const useDeleteParkingSlot = () => {
  ...
  return useMutation({
    mutationFn: async (id) => {
      const { data } = await apiClient.delete(`/parking/${id}`);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.PARKING });
...
export const useUpdateMoveRequest = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...moveData }) => {
      const { data } = await apiClient.put(`/move/${id}`, moveData);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.MOVE_REQUESTS });
...
export const useDeleteMoveRequest = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (id) => {
      const { data } = await apiClient.delete(`/move/${id}`);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.MOVE_REQUESTS });
...
export const useCreateDocument = () => {
  ...
  return useMutation({
    mutationFn: async (docData) => {
      const { data } = await apiClient.post(API_ENDPOINTS.UPLOAD_DOCUMENT, docData);
      return data;
    },
    onSuccess: () => {
...
export const useUpdateDocument = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...docData }) => {
      const { data } = await apiClient.put(`/documents/${id}`, docData);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.DOCUMENTS });
