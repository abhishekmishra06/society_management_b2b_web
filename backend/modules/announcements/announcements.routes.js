...
router.get('/', async (req, res) => {
  try {
    ...
    let q = 'SELECT * FROM announcements WHERE 1=1'; const p = [];
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (fields.length) { values.push(req.params.id); await pool.execute(`UPDATE announcements SET ${fields.join(',')} WHERE id=?`, values); }
