...
agent_communication:
    ...
    - agent: "main"
      message: "Phase 3: Enhanced Admin Panel - (1) Enhanced Society Onboarding with full profile (societyType, amenities, builder, billing). (2) Society Profile page at /admin/societies/{id} with tabs: Overview, Towers, Flats, Statistics. (3) Tower Management - CRUD towers within a society, auto-generate flats. (4) Flat Management - CRUD flats with filters (tower, status, type, search). (5) Advanced Filters on Societies (status, city, type), Users (role, society), Teams (society, status). Please test NEW API endpoints: GET /admin/societies/{id}/profile, POST/PUT/DELETE /admin/societies/{id}/towers, POST/PUT/DELETE /admin/societies/{id}/flats, POST /admin/societies with enhanced fields."